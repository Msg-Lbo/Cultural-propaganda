<template>
    <div id="category">
        <n-card style="margin-bottom: 16px">
            <n-tabs :default-value="12" :bar-width="50" justify-content="space-evenly" type="line"
                ...
                <n-tab-pane v-for="item in categoryList" :name="item.id" :tab="item.category_name" :key="item.id">
                    <n-scrollbar style="max-height: 90vh">
                        <div class="article-list-container">
                            <n-card class="article-content" v-for="item2 in ArticleList" :title="item2.article_title"
                                :key="item2.id" hoverable @click="toArticle(item2.id)">
                                <template #cover>
...
const getArticleListInCategoryId = async (id: number) => {
    ...
    const res = await getArticleListApi(page.value, pageSize.value, id)
    if (res.code === 200) {
