    ...
    <div id="user-info">
        <n-scrollbar style="max-height: 70vh">
            <div class="headimg">
                <span>用户头像:</span>
                <div class="upload">
                    <n-upload action="http://127.0.0.1:9090/api/user/upload-avatar" @before-upload="beforeUpload"
                        :show-file-list="false" @finish="handleFinish">
                        <img :src="avatar_url" alt="" srcset="">
                    </n-upload>
                </div>
            </div>
            <n-form ref="formRef" :model="userModel" :rules="userRules" label-placement="left" label-width="auto"
                require-mark-placement="left">
                <n-form-item path="id" label="UID">
                    <n-input v-model:value="userModel.id" style="max-width: 50%" disabled />
                </n-form-item>
                <n-form-item path="password" label="密码">
                    <n-button @click="openUpdate">修改密码</n-button>
                </n-form-item>
                <n-form-item path="account" label="账号">
                    <n-input v-model:value="userModel.account" style="max-width: 50%" disabled />
                </n-form-item>
                <n-form-item path="nickname" label="昵称">
                    <n-input v-model:value="userModel.nickname" style="max-width: 50%" />
                </n-form-item>
                <n-form-item path="email" label="邮箱">
                    <n-input v-model:value="userModel.email" style="max-width: 50%" />
                </n-form-item>
                ...
                <n-form-item ref="identity" label="身份">
                    <n-input v-model:value="userModel.identity" style="max-width: 50%" disabled />
                </n-form-item>
                <div style="display: flex; justify-content: flex-end">
                    <n-button round type="primary" @click="saveClick">
                        验证
                    </n-button>
                </div>
            </n-form>
